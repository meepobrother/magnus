...
export class ClientVisitor implements ast.Visitor {
  ...
  visitOperationDefinitionAst(
    ...
    const selections =
      node.selectionSet && node.selectionSet.visit(this, context);
    const variableDefinitions = node.variableDefinitions.map(def =>
  ...
  visitSelectionAst(
    node: ast.SelectionAst,
    context: any
  ): FieldJson | FragmentSpreadJson | InlineFragmentJson {
  ...
  visitFragmentSpreadAst(
    ...
    const directives: Directives = this.createDirectives(
      node.directives,
      context
    );
    return {
  ...
  visitVariableDefinitionAst(
    ...
    const defaultValue =
      node.defaultValue && node.defaultValue.visit(this, context);
    return {
      variable,
      type,
      directives: this.createDirectives(node.directives, context),
