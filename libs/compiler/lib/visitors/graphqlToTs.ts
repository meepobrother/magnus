import {
  ...
  SelectionSetAst,
  SelectionAst,
  ObjectTypeDefinitionAst,
  NamedTypeAst
} from "@notadd/magnus-graphql";
...
export class GraphqlToTs implements ast.Visitor {
  ...
  visitFieldDefinitionAst(
    ...
    if (this.isGraphql) {
      if (node.arguments) {
        if (parameters.length > 0) {
          res += `\t${name}<T>(${parameters.join(
            ", "
          )}, __selection?: string): Promise<T & ${type}>;\n`;
        } else {
    ...
    if (node.arguments) {
      if (parameters.length > 0) {
        res += `\t${name}(${parameters.join(
          ", "
        )}, __selection?: string): ${type};\n`;
      } else {
  ...
  visitInputObjectTypeDefinitionAst(
    node: InputObjectTypeDefinitionAst,
    context: string
  ) {
    this.isParameter = false;
  ...
  visitSelectionSetAst(
    node: SelectionSetAst,
    context: { operation: string; schema: ObjectTypeDefinitionAst }
  ): string {
    return (
      node.selections.map(selec => selec.visit(this, context)).join(``) + `\n`
    );
  }
  visitSelectionAst(
    node: SelectionAst,
  ...
  createType(
    ...
    if (ast) {
      if (ast instanceof ObjectTypeDefinitionAst) {
        ...
        if (ast) {
          if ((ast as any).name) {
            return `${(ast as any).name.visit(this, ``)}`;
          }
        }
      }
    }
  ...
  visitFieldAst(
    ...
    if (schema) {
      ...
      console.log(
        `类型错误: ${context.schema.name.value}不存在${fieldName}属性`
      );
    }
  }
