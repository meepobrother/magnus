...
export class ObjectLiteralElementLike extends Node<
...
> {
  visit(visitor: Visitor, context: any) {
    if (visitor.visitObjectLiteralElementLike) {
      ...
      throw new Error(
        `${visitor.name} 没有 visitObjectLiteralElementLike 方法`
      );
    }
  }
}
...
export class TsVisitor implements Visitor {
  ...
  visitObjectLiteralExpression(
    ...
    node.properties = context.properties.map(pro =>
      this.visitObjectLiteralElementLike(new ObjectLiteralElementLike(), pro)
    );
    return node;
  }
