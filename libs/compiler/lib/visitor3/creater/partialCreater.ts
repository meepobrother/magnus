...
export class PartialCreater extends BaseCreater {
  ...
  createClassDeclaration(name: string, node: ast.ClassDeclaration) {
    ...
    const members = node.members
      ...
      .filter(
        node =>
          !node.modifiers.find(it => ["private", "static"].includes(it.name))
    ...
    members.map(it => {
      ...
      const isDate = !!["CreateDateColumn", "UpdateDateColumn", "Time"].find(
        it => decorators.includes(it)
      );
      const isColumn = !![
        "Column",
      ...
      const isAuto = !!["CreateDateColumn", "UpdateDateColumn"].find(it =>
        decorators.includes(it)
      );
      ...
      if (isAuto) {
        ...
        input.fields.push(createInputValue(name, type, false, false, dec));
      } else if (isRelation) {
        // 关系
        const oneToMany = it.getDecorator("OneToMany")(expressionVisitor);
        if (it.type instanceof ast.TypeReferenceNode) {
          ...
          const createName = this.createName(
            it.type.elementType as any,
            this.context
          );
          if (createName) {
            const { name: astName, entity, namedType } = createName;
