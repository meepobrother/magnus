...
export class TsVisitor implements Visitor {
    ...
    visitCaseBlock(node: CaseBlock, context: ts.CaseBlock) {
        node.node = context;
        node.clauses = context.clauses.map(cla => this.visitCaseOrDefaultClause(undefined, cla))
        return node;
    }
    visitCaseOrDefaultClause(node: any, context: ts.CaseOrDefaultClause): CaseClause | DefaultClause {
        node.node = context;
    ...
    visitForInitializer(node: ForInitializer, context: ts.ForInitializer): Expression | VariableDeclarationList {
        ...
        if (ts.isVariableDeclarationList(context)) {
            ...
            return this.visitExpression(undefined, context)
        }
    }
     ...
     */
    visitStatement(node: any, context: ts.Statement): ClassDeclaration |
        ImportDeclaration | VariableStatement |
        FunctionDeclaration | InterfaceDeclaration |
        EnumDeclaration | TypeAliasDeclaration |
        ReturnStatement | IfStatement |
    ...
    visitImportEqualsDeclaration(node: ImportEqualsDeclaration, context: ts.ImportEqualsDeclaration) {
        node.moduleReference = this.visitModuleReference(undefined, context.moduleReference);
        if (context.name) {
            node.name = this.visitIdentifier(new Identifier(), context.name);
        }
        return node;
    }
    visitModuleReference(node: any, context: ts.ModuleReference) {
        return node;
    }
    ...
    visitClassElement(node: ClassElement, context: ts.ClassElement): ClassElement | PropertyDeclaration |
        MethodDeclaration | SemicolonClassElement | ConstructorDeclaration | GetAccessorDeclaration | SetAccessorDeclaration | Decorator {
        node.node = context;
    ...
    visitExpression(node: any, context: ts.Expression): StringLiteral |
        NumericLiteral | ObjectLiteralExpression | ArrayLiteralExpression |
        Identifier | AsExpression | AwaitExpression | ArrowFunction |
        BooleanLiteral | CallExpression | BinaryExpression |
        ElementAccessExpression | PropertyAccessExpression | PrefixUnaryExpression |
        ...
        if (ts.isStringLiteral(context)) {
            ...
            throw new Error(`visitExpression Error! ${context.kind}`)
        }
    }
    ...
    visitNoSubstitutionTemplateLiteral(node: NoSubstitutionTemplateLiteral, context: ts.NoSubstitutionTemplateLiteral) {
        node.text = context.text;
        node.hasExtendedUnicodeEscape = !!context.hasExtendedUnicodeEscape;
        node.isUnterminated = !!context.isUnterminated
        return node;
    ...
    visitArrayLiteralExpression(node: ArrayLiteralExpression, context: ts.ArrayLiteralExpression) {
        node.node = context;
        node.elements = context.elements.map(ele => this.visitExpression(undefined, ele))
        return node;
    }
    /**
     * type node start
    ...
    visitTypePredicateNode(node: TypePredicateNode, context: ts.TypePredicateNode) {
    ...
    visitCallExpression(node: CallExpression, context: ts.CallExpression) {
        ...
        node.arguments = context.arguments.map(arg => this.visitExpression(undefined, arg))
        if (context.typeArguments) {
